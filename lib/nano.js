...
Nano.prototype._processInclude = function(node, ctx, done) {
  ...
  var nano = this;
  async.eachSeries(node.nodes, function(node, cb) {
    if (node.type == 'def')
      return nano._processDef(node, ctx, cb);
    if (node.type == 'var')
      return nano._processVar(node, ctx, cb);
  }, function(err) {
    if (err) return done(err);
    // Read included file
...
Nano.prototype._processExpr = function(node, ctx, done) {
  var data = ctx.data;
  try {
    var expr = AngularExpressions.compile(node.expr);
    var value = expr(data);
    done(null, node.buffer ? (node.escape ? escapeHtml(value) : value) : '');
  } catch (e) {
    return done(e)
  }
};

Nano.prototype._processVar = function(node, ctx, done) {
  var data = ctx.data;
  try {
    var expr = AngularExpressions.compile(node.expr);
    data[node.name] = expr(data);
    done(null, '');
  } catch (e) {
    return done(e)
  }
