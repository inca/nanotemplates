...
Job.prototype._processNodes = function(nodes, ctx, done) {
  ...
  }, function(err, statementsBulk) {
    if (err) return done(err);
    done(null, [].concat.apply([], statementsBulk));
  });
};

Job.prototype._process_plain = function(text, done) {
  done(null, [
      'out.push(' + escapeJsString(text) + ')'
  ]);
};

Job.prototype._process_def = function(node, ctx, done) {
  var job = this;
  job._processNodes(node.nodes, ctx, function(err, statements) {
    if (err) return done(err);
    ctx.defs[node.name] = {
      mode: node.mode,
...
Job.prototype._process_block = function(node, ctx, done) {
  ...
  job._processNodes(node.nodes, ctx, function(err, statements) {
    ...
    switch (def.mode) {
      case 'append':
        return done(null, statements.concat(def.statements));
      case 'prepend':
        return done(null, def.statements.concat(statements));
      default:
        return done(null, def.statements);
    }
  });
};
...
Job.prototype._process_include = function(node, ctx, done) {
  ...
  async.mapSeries(node.nodes, function(node, cb) {
    ...
    if (err) return done(err);
    // Read included file
    job._processFile(node.file, ctx, function(err, statements) {
      if (err) return done(err);
      // Join all statements
      statements = [].concat.apply([], statementsBulk).concat(statements);
...
Job.prototype._process_expr = function(node, ctx, done) {
  ...
  try {
    ...
    if (node.buffer) {
      if (node.escape) {
        st = 'out.push(nano.escape($$[' + index + '](locals)))';
      } else {
