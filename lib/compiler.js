 ...
 *     See `loader.js` for more information on template loading.
 *
...
var Compiler = module.exports = exports = function(options) {
  ...
  this.load = typeof options.load == 'function' ?
    options.load : require('./loader').FileLoader(options);
...
Job.prototype.load = function(file, done) {
  if (file.indexOf('../') == 0)
    return done(null, new Error(file + ' is outside base directory'));
